...
# Safety limits for request assembly
const MAX_REQUEST_SIZE: int = 16 * 1024 * 1024  # 16 MB
const CLIENT_IDLE_TIMEOUT_MS: int = 30000  # 30s without completing a request
...
# Each entry: { "peer": StreamPeerTCP, "buffer": PackedByteArray, "started_ms": int }
var pending_clients: Array = []
...
func _poll_connections() -> void:
	...
	for i in range(pending_clients.size()):
		if _try_handle_client(pending_clients[i]):
...
func _try_handle_client(entry: Dictionary) -> bool:
	"""Accumulate bytes until a full HTTP request is available, then handle it.
	Returns true when the client should be removed from the pending queue."""
	...
	# Append any newly available bytes
	var bytes_available = client.get_available_bytes()
	if bytes_available > 0:
		var result = client.get_data(bytes_available)
		if result[0] == OK:
			entry["buffer"].append_array(result[1])
			debug_log("Buffered +%d bytes (total %d)" % [bytes_available, entry["buffer"].size()])
		else:
			debug_log("Failed to read client data: " + error_string(result[0]))
			client.disconnect_from_host()
			return true

	# Reject oversized requests early
	if entry["buffer"].size() > MAX_REQUEST_SIZE:
		_send_response(client, 400, {"error": "Request too large"})
		return true

	# Idle timeout while waiting for a complete request
	var elapsed = Time.get_ticks_msec() - int(entry.get("started_ms", 0))
	if elapsed > CLIENT_IDLE_TIMEOUT_MS:
		debug_log("Client idle timeout after %d ms" % elapsed)
		_send_response(client, 400, {"error": "Request timeout"})
		return true

	# Need at least the header block
	var split = _find_header_body_split(entry["buffer"])
	if split.x < 0:
		# Still waiting for headers
		return false

	var header_end: int = split.x
	var body_start: int = split.y
	var headers_text := entry["buffer"].slice(0, header_end).get_string_from_utf8()
	var content_length := _parse_content_length(headers_text)

	var total_needed: int
	if content_length >= 0:
		total_needed = body_start + content_length
		if total_needed > MAX_REQUEST_SIZE:
			_send_response(client, 400, {"error": "Request too large"})
			return true
		if entry["buffer"].size() < total_needed:
			# Wait for remaining body bytes
			debug_log("Waiting for body: have %d need %d" % [entry["buffer"].size(), total_needed])
			return false
	else:
		# No Content-Length: use whatever body we already have after headers
		total_needed = entry["buffer"].size()

	var request_text := entry["buffer"].slice(0, total_needed).get_string_from_utf8()
	debug_log("Complete request assembled (%d bytes)" % total_needed)
	request_count += 1
	_handle_client_request(client, request_text)
	debug_log("Request processed (#" + str(request_count) + ")")
	return true


func _find_header_body_split(buf: PackedByteArray) -> Vector2i:
	"""Return (header_end_index, body_start_index), or (-1, -1) if incomplete.
	header_end is the index of the first byte of the blank line terminator."""
	# Prefer CRLF CRLF (HTTP standard)
	for i in range(buf.size() - 3):
		if buf[i] == 13 and buf[i + 1] == 10 and buf[i + 2] == 13 and buf[i + 3] == 10:
			return Vector2i(i, i + 4)
	# Fallback: LF LF (some clients)
	for i in range(buf.size() - 1):
		if buf[i] == 10 and buf[i + 1] == 10:
			return Vector2i(i, i + 2)
	return Vector2i(-1, -1)


func _parse_content_length(headers_text: String) -> int:
	"""Return Content-Length value, or -1 if the header is absent."""
	for line in headers_text.split("\n"):
		var stripped = line.strip_edges()
		if stripped.to_lower().begins_with("content-length:"):
...
func _handle_client_request(client: StreamPeerTCP, request_text: String) -> void:
...
func _parse_http_request(request: String) -> Dictionary:
	# Normalize line endings so split works for CRLF and LF
	...
	# Parse body (JSON) — preserve exact body bytes/text after blank line
	var params = {}
	if body_start < lines.size():
		var body = "\n".join(lines.slice(body_start))
		# Do not strip trailing content that may be significant; only trim trailing CR noise
		...
		if body != "":
			...
			if error == OK:
				...
				debug_log("JSON parse failed: " + json.get_error_message())
